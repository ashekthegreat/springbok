<?php
	...
	//echo '<pre>';
	//print_r($_SERVER);
	$arrURLs = explode('?',$_SERVER['REQUEST_URI']);
	if(is_array($arrURLs) && count($arrURLs)>0)
	{#echo "here"; print_r($arrURLs);
	  #---utm_campaign variable value
	  if(isset($arrURLs[1]) && $arrURLs[1]!='')
	  {#echo "here1"; print_r($arrURLs);
	    $arrQS = explode('&',$arrURLs[1]);
	    if(is_array($arrQS) && count($arrQS)>0 && $arrQS[2]!='')
	    {#echo "here2"; print_r($arrQS);
	      $utm_campaign = explode('=',$arrQS[2]);
	      ...
	      {
		case 'googlesearch':
		$visitorSRC = 'Google Paid Search';
		$visitorSECSRC = 'PPC';
		break;
		
		case 'googledisplay':
		$visitorSRC = 'Google Display Network';
		$visitorSECSRC = 'PPC';
		break;
		
		case 'googleremar':
		$visitorSRC = 'Google Remarketing';
		$visitorSECSRC = 'PPC';
		break;
		
		case 'bingsearch':
		$visitorSRC = 'Bing Paid Search';
		$visitorSECSRC = 'PPC';
		break;
		
		case 'googlebrand':
		$visitorSRC = 'Google Paid Brand';
		$visitorSECSRC = 'PPC';
		break;
		
		case 'bingbrand':
		$visitorSRC = 'Bing Paid Brand'; 
		$visitorSECSRC = 'PPC';
		break;
		
		case 'googlemob':
		$visitorSRC = 'Google Mobile Search'; 
		$visitorSECSRC = 'PPC';
		break;
		...
		default :
		$visitorSRC = 'yellowspringbok.co.uk';
		$visitorSECSRC = 'WEB';
		break;
	      ...
	      #---SET COOKIE
	      setcookie ("YSSRCCOOKIE", $visitorSRC, time() + 3600,"/",".yellowspringbok.co.uk");
	      setcookie ("YSSECSRCCOOKIE", $visitorSECSRC, time() + 3600,"/",".yellowspringbok.co.uk");
	      
	    }else{ #echo "there2";
	      #--- READ COOKIE & JUDGE SRC
	      if(isset($_COOKIE['YSSRCCOOKIE']) && ($_COOKIE['YSSRCCOOKIE']!=''))
	      {#echo "TEST2";
		$visitorSRC 	= $_COOKIE['YSSRCCOOKIE'];
		$visitorSECSRC 	= $_COOKIE['YSSECSRCCOOKIE'];
	  ...
	  }else{# echo "there1"; print_r($_COOKIE);
	    #--- READ COOKIE & JUDGE SRC
	    if(isset($_COOKIE['YSSRCCOOKIE']) && ($_COOKIE['YSSRCCOOKIE']!='')) 
	    {#echo "TEST1";
	      $visitorSRC 	= $_COOKIE['YSSRCCOOKIE'];
	...
	#---Param 1
	$param1 = (isset($_GET['param1']) && $_GET['param1']!='')?$_GET['param1']:'Sell House Fast';
	
	#---Param 2
	$param2 = (isset($_GET['param2']) && $_GET['param2']!='')?$_GET['param2']:'We Buy Any House';
	
	#---Param 3
	$param3 = (isset($_GET['param3']) && $_GET['param3']!='')?$_GET['param3']:'Sell House Fast';
...
<body>
    <div class="main-container">
        <div class="wrapper">
            <header class="box clearfix text-center">
                <div class="header-item header-button pull-right col-sm-4">
                    <a href="tel:01614527543" class="sprite sprite-number2"></a>

                    <div class="header-call-button"><a href="tel:01614527543" class="sprite sprite-call-small"></a></div>
                </div>
                ...
                <div class="header-item other-logo">
                    ...
                    <img class="badgenopopup hidden-xs hidden-sm" src="http://yoshki.com/TSI/validation/130/0/logo.png" width="130" height="97">
                ...
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-sm-8">
                            ...
                            <div class="footer-notes hidden-xs">
                                <p><b>Sell House Fast</b> - Selling your house fast has never been this simple. With Yellow Springbok, you achieve the maximum market value for your property and sell in timescales that are suitable to your situation. </p>

                                <p><b>Sell House Fast</b> - Cash offers within 24 hours - Yellow Springbok is the UK's leading cash buyers; we are a national company with strong local presence. No matter why you need a quick sale, we provide a buying solution for any selling situation. We provide tailor-made solutions if you need to sell your house quick due to upsizing, downsizing, financial difficulties, separation, repossession, relocation, unwanted property inheritances or any other reason.</p>

                                <p><b>Sell House Fast</b> - Any condition, anywhere in the UK - We buy any type of property, any condition, anywhere in the UK. Even if it is not possible to get a fast house sale through traditional means, including derelict property, houses needing modernisation and downright ugly houses.</p>

                                <p><b>Sell House Fast</b> - 100% confidential process - Yellow Springbok and its associates adhere to strict code of conduct and high moral standards. Your enquiry will be handled quickly, compassionately and in strict confidence. We are a data protection registered and VAT registered company.</p>
                            </div>
                            <div class="footer-copyright">
                                © 2013 Yellow Springbok
                            </div>
                        </div>
    ...
    <div class="sprite sprite-floating_PPC"></div>
    <div class="floating-call"><a href="tel:01614527543" class="sprite sprite-call-small"></a></div>
    ...
    <script type="text/javascript">
        var _gaq = _gaq || [];
        _gaq.push(['_setAccount', 'UA-17783432-1']);
    ...
    <!-- Google Code for Vendor PPC Visitors -->
    <!-- Remarketing tags may not be associated with personally identifiable information or placed on pages related to sensitive categories. For instructions on adding this tag and more information on the above requirements, read the setup guide: google.com/ads/remarketingsetup -->
    <div style="display:none;">
        <script type="text/javascript">
            /* <![CDATA[ */
            var google_conversion_id = 1037040439;
            var google_conversion_label = "pBidCNmrzAIQt_a_7gM";
    ...
    <script type="text/javascript">
        (function () {
            ...
            _fbq.push(['addPixelId', '591173200998289']);
        })();
